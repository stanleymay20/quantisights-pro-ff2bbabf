import {
  ...
  Users, BarChart3, Rocket, CheckCircle2, Brain, MapPin, Crosshair
} from "lucide-react";
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4">
        <p className="text-lg font-semibold text-foreground">Executives make high-stakes decisions with no institutional memory, no calibration, and no audit trail.</p>
        <ul className="space-y-3 text-sm text-muted-foreground">
          <li className="flex items-start gap-2"><span className="text-destructive font-bold mt-0.5">✕</span> <strong className="text-foreground">73% of executives</strong> are systematically overconfident in strategic forecasts (HBR 2023)</li>
          <li className="flex items-start gap-2"><span className="text-destructive font-bold mt-0.5">✕</span> No enterprise tool tracks whether past predictions were accurate — or adjusts future confidence</li>
          <li className="flex items-start gap-2"><span className="text-destructive font-bold mt-0.5">✕</span> Boards lack defensible decision trails — exposing organizations to governance risk</li>
          <li className="flex items-start gap-2"><span className="text-destructive font-bold mt-0.5">✕</span> PE/VC firms can't measure portfolio management quality across companies</li>
        </ul>
        ...
        <div className="p-4 rounded-lg bg-destructive/5 border border-destructive/20">
          <p className="text-sm font-semibold text-destructive">$2.3 Trillion in annual value destruction from ungoverned strategic decisions</p>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-center gap-1 text-xs font-semibold py-3">
          {["Decision", "Prediction", "Outcome", "Calibration", "Better Decisions"].map((step, i) => (
            <div key={step} className="flex items-center gap-1">
              <span className="px-2.5 py-1.5 rounded-md bg-primary/10 text-primary border border-primary/20">{step}</span>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={t.tier} className="p-3 rounded-lg border border-border flex items-center justify-between">
              ...
              <p className="font-bold text-primary whitespace-nowrap ml-3">{t.price}</p>
            </div>
          ))}
        </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-center py-2 px-2 font-semibold text-primary">Quantivis</th>
                <th className="text-center py-2 px-2 font-semibold">Palantir</th>
                <th className="text-center py-2 px-2 font-semibold">Tableau</th>
                <th className="text-center py-2 px-2 font-semibold">Mosaic</th>
              </tr>
            </thead>
            <tbody className="text-muted-foreground">
              {[
                { f: "Decision → Outcome loop", q: true, p: false, t: false, m: false },
                { f: "Calibrated confidence scores", q: true, p: false, t: false, m: false },
                { f: "Board-defensible audit trail", q: true, p: "Partial", t: false, m: false },
                { f: "Cognitive bias detection", q: true, p: false, t: false, m: false },
                { f: "Time to first insight", q: "5 min", p: "Weeks", t: "Days", m: "Hours" },
                { f: "Monthly cost", q: "€99+", p: "€50K+", t: "€70/user", m: "€800+" },
              ].map((row) => (
                <tr key={row.f} className="border-b border-border/30">
                  ...
                  {[row.q, row.p, row.t, row.m].map((v, i) => (
                    <td key={i} className={`text-center py-1.5 px-2 ${i === 0 ? "bg-primary/5" : ""}`}>
