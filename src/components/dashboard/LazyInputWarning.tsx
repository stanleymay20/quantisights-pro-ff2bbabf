...
export function detectLazyInput(decisions: Decision[]): LazyPattern[] {
  const probs = decisions
    .map(d => d.confidence_at_decision ?? d.capped_confidence ?? null)
    .filter((p): p is number => p !== null);

  if (probs.length < 5) return [];

  const patterns: LazyPattern[] = [];

  // 1. Probability Clustering: >60% within ±5pp of each other
...
const LazyInputWarning = ({ decisions }: LazyInputWarningProps) => {
  const patterns = useMemo(() => detectLazyInput(decisions), [decisions]);
    ...
    <Card className={`${hasCritical ? "border-destructive/30 bg-destructive/5" : "border-warning/30 bg-warning/5"}`}>
      <CardContent className="p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">Probability Input Quality Alert</span>
          <Tooltip>
            <TooltipTrigger>
              <Info className="w-3.5 h-3.5 text-muted-foreground" />
            </TooltipTrigger>
            <TooltipContent className="max-w-xs">
              <p className="text-xs">These patterns suggest probability assignments may not reflect genuine beliefs. Calibration accuracy depends on honest, differentiated estimates.</p>
            </TooltipContent>
          </Tooltip>
        </div>
