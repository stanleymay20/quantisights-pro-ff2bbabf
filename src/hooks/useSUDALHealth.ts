  ...
  const compute = useCallback(async () => {
    ...
    try {
      const [
        ...
        supabase.from("decision_ledger").select("id, status, confidence, decided_at").eq("organization_id", organizationId).limit(500),
        supabase.from("decision_outcomes").select("id, actual_outcome").eq("organization_id", organizationId).limit(500),
