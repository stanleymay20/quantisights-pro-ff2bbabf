// tests/evidence/pipelines/lib/ai-pipeline.mjs
// Shared EE-5 mocked-AI evidence projection.

import { readFileSync, existsSync } from "node:fs";
import { STATUS } from "../../lib/taxonomy.mjs";
import { AI_CONTROL_INDEX, AI_PIPELINE_CONTROL_IDS } from "./ai-controls.mjs";
...
function loadAdapterResults(resultsPath) {
  if (!resultsPath) {
    return { ok: false, code: "MISSING_ADAPTER_RESULTS", message: "EVIDENCE_AI_RESULTS env var not set" };
  }
...
export function buildPipelineEvidence(pipeline, adapterResults) {
  ...
  const required = AI_PIPELINE_CONTROL_IDS[pipeline] ?? [];
  ...
  for (const controlId of required) {
    const control = AI_CONTROL_INDEX[controlId];
    ...
    if (!rec) {
      failures.push({
        ...
        status: STATUS.FRAMEWORK_INVALID,
        reason: `Missing required EE-5 control ${controlId}`,
        blocking: true,
      });
      continue;
    }
    ...
    if (!VALID_ADAPTER_STATUSES.has(adapterStatus)) {
      failures.push({
        ...
        status: STATUS.FRAMEWORK_INVALID,
        reason: `Invalid EE-5 control status "${adapterStatus}"`,
        blocking: true,
      });
      continue;
    }
    ...
    if (adapterStatus === "SKIP") {
      warnings.push({
        ...
        message: `${controlId} skipped; cannot claim full mocked AI coverage`,
      });
      continue;
    }
...
export async function verifyPipeline(pipeline) {
  const loaded = loadAdapterResults(process.env.EVIDENCE_AI_RESULTS);
